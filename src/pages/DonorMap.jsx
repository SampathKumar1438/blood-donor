  ...
  useEffect(() => {
    ...
    const fetchDonors = async () => {
      try {
        ...
        const response = await donorService.getAllDonors(apiFilters);
        
        // Check if we have donors with coordinates
        let donorsData = response.data || [];
        
        // Filter out donors without coordinates
        donorsData = donorsData.filter(donor => donor.coordinates && 
          Array.isArray(donor.coordinates) && 
          donor.coordinates.length === 2);
        
        if (donorsData.length === 0) {
          // If we don't have real data with coordinates, fall back to mock data for demo
          const mockDonors = [
            { 
              id: 1, 
              name: 'John Doe', 
          ...
          // Add a warning that we're using mock data
          console.warn('Using mock donor data - no real data with coordinates available');
        } else {
        ...
        // In case of error, show mock data for demo purposes
        const mockDonors = [
          { 
            id: 1, 
            name: 'John Doe', 
